...
def get_profile_data(email=None,mobile_no=None,reference_doctype=None,reference_docname=None,resync=False):
	...
	try:
		if(resync):
			...
			queued_request = frappe.get_list('Profile Enrich Request',filters={"status":"Queued"},or_filters=query_object,fields='["*"]',order_by="creation desc",limit_page_length=1)
			if(len(queued_request)>0):
				response = sync_profile_enrich_request(queued_request[0])
			else:
				request_data = frappe.get_list("Profile Enrich Request",filters={"status":"Success"},or_filters = query_object,fields='["*"]',order_by="creation desc",limit_page_length=1)
...
def sync_profile_enrich_request(profile_enrich_request):
	try:
		...
		prev_sync_request = frappe.get_list('Profile Enrich Archive',filters={'reference_site':current_site_name,'reference_profile_enrich_request':profile_enrich_request.name,'status':'Success'},fields=["*"],order_by="creation desc",limit_page_length=1)
		if len(prev_sync_request)>0:
			...
			prev_sync_request = frappe.get_list('Profile Enrich Archive',filters={'status':'Success'},or_filters=query_object,fields=["*"],order_by="creation desc",limit_page_length=1)
			if(len(prev_sync_request)>0):
				archive_data_record = prev_sync_request[0]
				profile_enrich = json.loads(archive_data_record.request_data)
			else:
		...
		response = update_profile_enrich_archive_request(profile_enrich,archive_data_record)
